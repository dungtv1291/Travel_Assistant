...
export default function ProfileScreen() {
  ...
  const [profileCounts, setProfileCounts] = useState<{ trips: number; bookings: number; favorites: number } | null>(null);

  useEffect(() => {
    authService.loadProfile().then(({ user: remoteUser, counts }) => {
      if (remoteUser) updateUser(remoteUser);
      if (counts) setProfileCounts(counts);
    }).catch(() => {});
  }, []);
  ...
  const storeBookingsCount = hotelBookings.length + transportBookings.length;
  const totalBookings = profileCounts?.bookings ?? storeBookingsCount;
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.statsCard}>
          <TouchableOpacity style={styles.statItem} onPress={() => router.push('/(tabs)/trips')}>
            ...
            <Text style={styles.statValue}>{profileCounts?.trips ?? savedTrips.length}</Text>
          ...
          <TouchableOpacity style={styles.statItem} onPress={() => router.push('/(tabs)/bookings')}>
            ...
            <Text style={[styles.statValue, { color: Colors.accent }]}>{profileCounts?.bookings ?? storeBookingsCount}</Text>
          ...
          <TouchableOpacity style={styles.statItem} onPress={() => router.push('/(tabs)/trips')}>
            ...
            <Text style={[styles.statValue, { color: '#D97706' }]}>{profileCounts?.favorites ?? favorites.length}</Text>
          ...
          <View style={styles.menuCard}>
            <TouchableOpacity style={[styles.menuRow, styles.menuRowBorder]} onPress={() => router.push('/(tabs)/trips')}>
              ...
              <View style={styles.menuRight}>
                {(profileCounts?.trips ?? savedTrips.length) > 0 && <View style={styles.badgePill}><Text style={styles.badgeText}>{profileCounts?.trips ?? savedTrips.length}</Text></View>}
            ...
            <TouchableOpacity style={[styles.menuRow, styles.menuRowBorder]} onPress={() => router.push('/(tabs)/bookings')}>
              ...
              <View style={styles.menuRight}>
                {(profileCounts?.bookings ?? storeBookingsCount) > 0 && <View style={[styles.badgePill, { backgroundColor: Colors.accent }]}><Text style={styles.badgeText}>{profileCounts?.bookings ?? storeBookingsCount}</Text></View>}
                <Ionicons name="chevron-forward" size={16} color={Colors.textMuted} />
              </View>
            </TouchableOpacity>
            <TouchableOpacity style={styles.menuRow} onPress={() => router.push('/(tabs)/trips')}>
              ...
              <View style={styles.menuRight}>
                {(profileCounts?.favorites ?? favorites.length) > 0 && <View style={[styles.badgePill, { backgroundColor: '#D97706' }]}><Text style={styles.badgeText}>{profileCounts?.favorites ?? favorites.length}</Text></View>}
                <Ionicons name="chevron-forward" size={16} color={Colors.textMuted} />
              </View>
            </TouchableOpacity>
          </View>
