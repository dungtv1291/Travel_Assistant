// ─── Room Badges editor ─────────────────────────────────────────────────────
// Inline CRUD panel for hotel_room_badges.
// Same UX pattern as hotels/HotelBadgesEditor but keyed by room_type_id.

import { useEffect, useState } from 'react';
import { Plus, Pencil, Trash2, Check, X } from 'lucide-react';
import { Input } from '@/components/ui/Input';
import { Button } from '@/components/ui/Button';
import { roomBadgesService, type RoomBadgePayload } from '@/services/hotel-rooms.service';
import { getErrorMessage } from '@/lib/api';
import type { HotelRoomBadge } from '@/types';

interface RoomBadgesEditorProps {
  roomId: number;
}

const EMPTY_ROW: RoomBadgePayload = {
  label_ko: '',
  label_en: '',
  label_vi: '',
  badge_group: null,
...
export function RoomBadgesEditor({ roomId }: RoomBadgesEditorProps) {
  const [badges, setBadges] = useState<HotelRoomBadge[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  const [editingId, setEditingId] = useState<number | null>(null);
  const [editDraft, setEditDraft] = useState<RoomBadgePayload>(EMPTY_ROW);
  const [editSaving, setEditSaving] = useState(false);

  const [showAdd, setShowAdd] = useState(false);
  const [addDraft, setAddDraft] = useState<RoomBadgePayload>(EMPTY_ROW);
  ...
  async function load() {
    ...
    setError('');
    try {
      const data = await roomBadgesService.list(roomId);
      setBadges(data);
    } catch (err) {
  ...
  async function submitAdd() {
    ...
    setAddSaving(true);
    try {
      await roomBadgesService.create(roomId, addDraft);
      setShowAdd(false);
      await load();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  function startEdit(badge: HotelRoomBadge) {
    setShowAdd(false);
  ...
  async function submitEdit(badgeId: number) {
    setEditSaving(true);
    try {
      await roomBadgesService.update(roomId, badgeId, editDraft);
      setEditingId(null);
      await load();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  async function handleDelete(badge: HotelRoomBadge) {
    if (!window.confirm(`Delete badge "${badge.label_en}"?`)) return;
    try {
      await roomBadgesService.delete(roomId, badge.id);
      await load();
    } catch (err) {
      setError(getErrorMessage(err));
    }
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
        <div>
          <h3 className="text-sm font-semibold text-slate-800">Room Badges</h3>
          <p className="text-xs text-slate-500 mt-0.5">
            Feature labels shown on room cards (e.g. Sea View, Balcony).
          </p>
        </div>
        {!showAdd && (
          ...
          {badges.map((badge) =>
            editingId === badge.id ? (
              <div key={badge.id} className="px-5 py-3 bg-slate-50 space-y-3">
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <Input
                    ...
                    placeholder="Hướng biển"
                  />
                  <Input
                    label="Group"
                    value={editDraft.badge_group ?? ''}
                    onChange={(e) => setEditDraft((d) => ({ ...d, badge_group: e.target.value || null }))}
                    placeholder="view"
                  />
                </div>
              ...
              <div key={badge.id} className="flex items-center gap-4 px-5 py-2.5">
                ...
                <span className="text-xs text-slate-400 w-20 truncate">{badge.badge_group ?? '—'}</span>
          ...
          {showAdd && (
            <div className="px-5 py-3 bg-blue-50 border-t border-blue-100 space-y-3">
              <div className="grid grid-cols-4 gap-3">
                ...
                <Input
                  label="Group"
                  value={addDraft.badge_group ?? ''}
                  onChange={(e) => setAddDraft((d) => ({ ...d, badge_group: e.target.value || null }))}
                  placeholder="view"
                />
              </div>
              <div className="flex items-center gap-2">
