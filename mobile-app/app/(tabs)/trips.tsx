...
export default function TripsScreen() {
  ...
  const { savedTrips, favorites, saveTrip } = useTripsStore();
  const [activeTab, setActiveTab] = useState<Tab>('saved');
  const [loadedTrips, setLoadedTrips] = useState<Trip[] | null>(null);
  const { t } = useTranslation();
  const { formatPrice, formatDate, formatDuration } = useFormatter();

  // Load saved trips from backend on mount; fall back to local store on error
  useEffect(() => {
    aiPlannerService.getSavedTrips()
      .then(trips => {
        setLoadedTrips(trips);
        // Sync backend saved trips into local store for offline access
        trips.forEach(trip => {
      ...
      .catch(() => setLoadedTrips(savedTrips));
  }, []);

  // Use backend-loaded trips if available, otherwise fall back to local store
  const displayedTrips = loadedTrips ?? savedTrips;
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={styles.summaryRow}>
        <View style={styles.summaryChip}>
          ...
          <Text style={styles.summaryNum}>{displayedTrips.length}</Text>
