...
export default function HomeScreen() {
  ...
  const [upcomingBooking, setUpcomingBooking] = useState<HotelBooking | null>(null);
  const [transports, setTransports] = useState<TransportVehicle[]>([]);
  const [refreshing, setRefreshing] = useState(false);

  const loadData = async () => {
    const [dests, htls, fls, bookings, vehicles] = await Promise.all([
      destinationsService.getFeatured(),
      hotelsService.getRecommended(),
      flightsService.getDeals(),
      bookingsService.getHotelBookings(),
    ...
    setUpcomingBooking(confirmed[0] ?? null);
    setTransports(vehicles.slice(0, 8));
  };
  ...
  const transportTiles = useMemo(() => {
    const all = [
      { icon: '🛬', type: 'airport_pickup' as TransportType, titleKey: 'home.transportAirport', subKey: 'home.transportAirportSub', bg: Colors.primaryLight },
      { icon: '🚗', type: 'private_car'    as TransportType, titleKey: 'home.transportPrivate',  subKey: 'home.transportPrivateSub',  bg: '#DCFCE7' },
      { icon: '🛵', type: 'scooter'        as TransportType, titleKey: 'home.transportScooter',  subKey: 'home.transportScooterSub',  bg: '#FEF3C7' },
      { icon: '🚌', type: 'day_tour'       as TransportType, titleKey: 'home.transportDayTour',  subKey: 'home.transportDayTourSub',  bg: Colors.accentLight },
    ];
    if (transports.length === 0) return all;
    const available = new Set(transports.map(v => v.type));
    return all.filter(tile => available.has(tile.type));
  }, [transports, Colors]);
