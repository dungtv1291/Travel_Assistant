...
import type { Hotel, HotelReview, RoomType } from '../../types/hotel.types';
  ...
  useEffect(() => {
    if (id) {
      Promise.all([hotelsService.getById(id), hotelsService.getRooms(id)]).then(([h, r]) => {
        ...
        setLoading(false);
        // Lazy-load reviews after main content is shown
        hotelsService.getReviews(id).then(setReviews);
      });
    }
  }, [id]);
          ...
          <View style={styles.content}>
            ...
            {activeTab === 3 && (
              <View style={styles.section}>
                ...
                {/* Review cards */}
                {(reviews.length > 0 ? reviews : (hotel.reviews ?? [])).map((rev: HotelReview, i: number) => (
