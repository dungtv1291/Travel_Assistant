...
import { MapPinned, Search, ChevronLeft, ChevronRight, Pencil, Trash2 } from 'lucide-react';
...
function RowActions({ opt, onDeleted }: { opt: TransportPickupOption; onDeleted: () => void }) {
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <Link to={`/transport-pickup-options/${opt.id}/edit`}>
...
const baseColumns: Column<TransportPickupOption>[] = [
  {
    key: 'id',
    header: 'ID',
    width: '56px',
  ...
  },
  {
    key: 'option_key',
    header: 'Key',
    render: (r) => <code className="text-xs text-slate-500 bg-slate-50 px-1.5 py-0.5 rounded">{r.option_key}</code>,
  },
  {
    key: 'label_en',
    header: 'Label (EN)',
    render: (r) => (
      <Link to={`/transport-pickup-options/${r.id}/edit`} className="font-medium text-blue-600 hover:underline">
        {r.label_en}
      </Link>
    ),
  },
  ...
  },
  {
    key: 'description_en',
    header: 'Description',
    render: (r) => r.description_en ? (
      <span className="text-xs text-slate-500 line-clamp-1">{r.description_en}</span>
...
const columnsWithActions = (onDeleted: () => void): Column<TransportPickupOption>[] => [
  ...baseColumns,
  {
    key: 'id' as keyof TransportPickupOption,
    header: '',
    width: '80px',
...
export function TransportPickupOptionsPage() {
  const [items, setItems] = useState<TransportPickupOption[]>([]);
  ...
  async function loadWith(params: { search?: string; transport_service_id?: string | number; page?: number }) {
    ...
    setError('');
    try {
      const res = await transportPickupOptionsService.list({
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Manage pickup point choices for transport services"
        actions={
          <Link to="/transport-pickup-options/new">
      ...
      {items.length === 0 && !loading ? (
        <EmptyState
          icon={MapPinned}
          title="No pickup options found"
          description={search || transportId ? 'No options match your filters.' : 'Add your first pickup option.'}
          action={
            !search && !transportId ? (
              <Link to="/transport-pickup-options/new">
                <Button size="sm">+ New option</Button>
        ...
        <>
          <Table
            ...
            rowKey={(r) => r.id}
            loading={loading}
            emptyMessage="No pickup options found."
          />
          {totalPages > 1 && (
