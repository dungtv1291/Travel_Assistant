// ─── Badges editor ────────────────────────────────────────────────────────
// Inline CRUD panel for destination_badges.
// Shows existing badges in a table; allows add, inline-edit, delete.

import { useEffect, useState } from 'react';
import { Plus, Pencil, Trash2, Check, X } from 'lucide-react';
import { Input } from '@/components/ui/Input';
import { Button } from '@/components/ui/Button';
import { destinationBadgesService, type BadgePayload } from '@/services/destinations.service';
import { getErrorMessage } from '@/lib/api';
import type { DestinationBadge } from '@/types';

interface BadgesEditorProps {
  destinationId: number;
}

const EMPTY_ROW: BadgePayload = {
  label_ko: '',
  label_en: '',
  label_vi: '',
  badge_type: null,
...
export function BadgesEditor({ destinationId }: BadgesEditorProps) {
  const [badges, setBadges] = useState<DestinationBadge[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');

  // Row being edited – null means none
  const [editingId, setEditingId] = useState<number | null>(null);
  const [editDraft, setEditDraft] = useState<BadgePayload>(EMPTY_ROW);
  const [editSaving, setEditSaving] = useState(false);

  // Add form
  const [showAdd, setShowAdd] = useState(false);
  const [addDraft, setAddDraft] = useState<BadgePayload>(EMPTY_ROW);
  ...
  async function fetch() {
    ...
    setError('');
    try {
      const data = await destinationBadgesService.list(destinationId);
      setBadges(data);
    } catch (err) {
  ...
  async function submitAdd() {
    ...
    setAddSaving(true);
    try {
      await destinationBadgesService.create(destinationId, addDraft);
      setShowAdd(false);
      await fetch();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  function startEdit(badge: DestinationBadge) {
    setShowAdd(false);
  ...
  async function submitEdit(badgeId: number) {
    setEditSaving(true);
    try {
      await destinationBadgesService.update(destinationId, badgeId, editDraft);
      setEditingId(null);
      await fetch();
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  async function handleDelete(badge: DestinationBadge) {
    if (!window.confirm(`Delete badge "${badge.label_en}"?`)) return;
    try {
      await destinationBadgesService.delete(destinationId, badge.id);
      await fetch();
    } catch (err) {
      setError(getErrorMessage(err));
    }
      ...
      <div className="flex items-center justify-between border-b border-slate-100 px-5 py-3.5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Labels shown on destination cards (e.g. 유네스코, 해변).
          </p>
        </div>
        {!showAdd && (
