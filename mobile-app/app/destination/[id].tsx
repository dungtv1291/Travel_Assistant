...
import type { MappedWeather, MappedTips } from '../../services/adapters/destination.adapter';
...
const DEFAULT_TIPS = [
  '현지 통화(동)로 미리 환전하세요.',
  '그랩(Grab) 앱으로 이동하면 편리합니다.',
  '자외선이 강하니 선크림을 항상 챙기세요.',
  '음식과 물로 인한 배탈에 주의하세요.',
  '여행자 보험에 꼭 가입하세요.',
  '현지 유심을 구입하면 저렴하게 데이터를 쓸 수 있습니다.',
];
...
export default function DestinationDetailScreen() {
  ...
  const [weatherData, setWeatherData] = useState<MappedWeather | null>(null);
  const [tipsData, setTipsData] = useState<MappedTips | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLoading(false);
      // Load hotels, weather, and tips after the main content is shown
      if (dest) {
        const fallbackBestSeason = dest.bestSeason ?? dest.bestTimeToVisit ?? '';
        Promise.all([
          destinationsService.getHotelsByDestination(id),
          destinationsService.getWeatherData(id, fallbackBestSeason),
          destinationsService.getTipsData(id, dest.travelTipsKo ?? DEFAULT_TIPS),
        ]).then(([htls, wx, tips]) => {
          setDestHotels(htls);
          setWeatherData(wx);
          setTipsData(tips);
      ...
      <View style={styles.container}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.content}>
            ...
            {/* 날씨 Tab */}
            {activeTab === 3 && (() => {
              // Use live weatherData when loaded, fall back to destination.weather
              const wx = weatherData?.weather ?? destination.weather;
              const bestSeason = weatherData?.bestSeason ?? destination.bestSeason ?? destination.bestTimeToVisit;
              const seasons = weatherData?.seasonBlocks ?? [
                { seasonKey: 'spring', label: '봄', monthsLabel: '3~4월', note: '여행 성수기', icon: '🌸' },
                { seasonKey: 'summer', label: '여름', monthsLabel: '5~8월', note: '고온 다습', icon: '☀️' },
                { seasonKey: 'autumn', label: '가을', monthsLabel: '9~10월', note: '최적 시즌', icon: '🍂' },
                { seasonKey: 'winter', label: '겨울', monthsLabel: '11~2월', note: '선선하고 건조', icon: '❄️' },
              ];
              const packing = weatherData?.packingItems ?? [
              ...
              <View style={styles.section}>
                ...
                >
                  ...
                  <View style={styles.weatherHeroTop}>
                    <View>
                      <Text style={styles.weatherTempHero}>{wx.temperature}°C</Text>
                      <Text style={styles.weatherCondHero}>
                        {wx.description || wx.condition}
                      </Text>
                    </View>
                    <Text style={styles.weatherIconHero}>{wx.icon ?? '🌤️'}</Text>
                  </View>
                  <View style={styles.weatherHeroStats}>
                    <View style={styles.weatherHeroStat}>
                      <Text style={styles.weatherHeroStatNum}>{wx.humidity}%</Text>
                      <Text style={styles.weatherHeroStatLabel}>{t('destination.humidity')}</Text>
                    </View>
                    <View style={styles.weatherHeroStatDivider} />
                    <View style={styles.weatherHeroStat}>
                      <Text style={styles.weatherHeroStatNum}>{t('destination.rainfallAmount', { amount: wx.rainfall ?? 0 })}</Text>
                      <Text style={styles.weatherHeroStatLabel}>{t('destination.rainfall')}</Text>
                    </View>
                    <View style={styles.weatherHeroStatDivider} />
                    <View style={styles.weatherHeroStat}>
                      <Text style={styles.weatherHeroStatNum}>
                        {wx.temperature >= 32 ? t('destination.uvStrong') : wx.temperature >= 26 ? t('destination.uvMedium') : t('destination.uvWeak')}
                      </Text>
                ...
                <View style={styles.bestTimeCard}>
                  ...
                  <Text style={styles.bestTimeValue}>{bestSeason}</Text>
                  <Text style={styles.bestTimeHint}>{t('destination.bestTimeHint')}</Text>
                </View>
                ...
                <View style={styles.seasonCard}>
                  ...
                  <View style={styles.seasonRow}>
                    {seasons.map((s) => (
                      <View key={s.seasonKey} style={styles.seasonItem}>
                        <Text style={styles.seasonIcon}>{s.icon}</Text>
                        <Text style={styles.seasonLabel}>{s.label}</Text>
                        <Text style={styles.seasonMonths}>{s.monthsLabel}</Text>
                        <Text style={styles.seasonNote}>{s.note}</Text>
                      </View>
                    ))}
                  </View>
                </View>
            ...
            {/* 여행 팁 Tab */}
            {activeTab === 4 && (() => {
              const tips = tipsData?.tips ?? destination.travelTipsKo ?? DEFAULT_TIPS;
              const apps = tipsData?.essentialApps ?? [
                { icon: '🚗', name: 'Grab', descKey: 'destination.apps.grab', subtitle: '' },
                { icon: '🗺️', name: 'Google Maps', descKey: 'destination.apps.googleMaps', subtitle: '' },
                { icon: '🌐', name: 'DeepL', descKey: 'destination.apps.deepl', subtitle: '' },
                { icon: '💱', name: 'XE Money', descKey: 'destination.apps.xeMoney', subtitle: '' },
              ];
              return (
              <View style={styles.section}>
                  ...
                  {apps.map(app => (
                    <View key={app.name} style={styles.appItem}>
                      ...
                      <Text style={styles.appDesc}>
                        {app.descKey ? t(app.descKey as any) : app.subtitle}
                      </Text>
                    </View>
                  ))}
                </View>
