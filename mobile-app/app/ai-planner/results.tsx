...
export default function AIPlannerResultsScreen() {
  ...
  const { destination, days, travelStyle, travelerType, budget, pace, interests, tripId } = useLocalSearchParams<Record<string, string>>();
  ...
  // Build the generate request from URL params (reused for generate and regenerate)
  const buildRequest = () => ({
  ...
  useEffect(() => {
    if (tripId) {
      // Reopen saved itinerary by ID
      aiPlannerService.getTripById(tripId).then(t => {
        setTrip(t);
        setSaved(true);
        setLoading(false);
      });
    } else {
      aiPlannerService.generateItinerary(buildRequest()).then(t => {
        setTrip(t);
        setLoading(false);
      });
    }
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Persist to backend if we have a real trip ID from the API
      if (trip.id && !trip.id.startsWith('mock-')) {
        await aiPlannerService.saveTrip(trip.id);
      }
      saveTrip(trip);
      setSaved(true);
      Alert.alert(t('aiPlanner.savedSuccess'), t('aiPlanner.savedSuccessHint'));
    } catch {
      // saveTrip to local store even if API call fails
      saveTrip(trip);
      setSaved(true);
      Alert.alert(t('aiPlanner.savedSuccess'), t('aiPlanner.savedSuccessHint'));
    } finally {
  ...
  const handleRegenerate = async () => {
    ...
    let newTrip: Trip;
    if (trip?.id && !trip.id.startsWith('mock-')) {
      // Regenerate existing saved trip via the backend
      newTrip = await aiPlannerService.regenerateTripById(trip.id, 'full');
    } else {
      newTrip = await aiPlannerService.generateItinerary(buildRequest());
    }
    setTrip(newTrip);
    setLoading(false);
  };
